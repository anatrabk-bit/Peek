...
type PeekRatingBadgeProps = {
  summary: UserRatingSummary;
  userId?: string;
  compact?: boolean;
};

export function PeekRatingBadge({
  summary,
  userId,
  compact = false
}: PeekRatingBadgeProps) {
  ...
  const badge = (
    ...
    >
      <span>Peek {summary.averageScore}/5</span>
  ...
  return (
    <Link href={`/users/${userId}?as=peek`} className="inline-block">
      {badge}
    </Link>
  );
}
